...
class MarvelmindHedge (Thread):
    def __init__ (self, adr=None, tty="COM3", baud=9600, maxvaluescount=3, debug=False, recieveUltrasoundPositionCallback=None, recieveImuRawDataCallback=None, recieveImuDataCallback=None, recieveUltrasoundRawDataCallback=None):
        self.tty = tty  # serial
    ...
    def print_position(self):
        if (self.position()[5]):
            print("1")
            tsec= int(math.trunc(self.position()[4]/1000.0))
                ...
                try:
                    ...
                    while (readChar != None) and (readChar != '') and (not self.terminationRequired):
                        ...
                        if (pktHdrOffset >= 0 and len(bufferList) > pktHdrOffset + 4 and pktHdrOffset<220):
                            #print(bufferList)                                                                   ##Deze lijn stond in commentaar

...
class Create2(object):
    ...
    def buttons(self, button):
        """
        Toggle buttons directly

        button: "clean", "spot" or "dock"
        """
        if button == "clean":
            self.SCI.write(bytes([165, 1]))
        elif button == "spot":
            self.SCI.write(bytes([140, 1, 134]))
        elif button == "dock":
            self.SCI.write(bytes([140, 1, 143]))
        else:
            self.SCI.write(bytes([165, 0]))